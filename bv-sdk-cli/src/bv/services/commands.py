...
def publish_package(config_path: Path, publish_dir: Path, dry_run: bool, bump: str = "patch") -> Path:
    ...
    # Bump version (only if not dry_run)
    next_version = bump_semver(cfg.version, bump)
    cfg.version = next_version
    
    if dry_run:
        # In dry-run mode, return the expected package path without modifying files
        package_path = Path("dist") / f"{cfg.name}-{cfg.version}.bvpackage"
        return package_path

    # Only write the bumped version if not dry_run
    ...
    package_path = build_package(config_path=config_path, output=None, dry_run=False)
