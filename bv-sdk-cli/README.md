...
- **How it differs from the runner**: The SDK CLI runs on developer machines. It sets `BV_SDK_RUN=1` when invoking code so that `bv.runtime.*` is allowed. The production Runner uses a local sqlite context store (`~/.bv/runner_context.db`) for authenticated runtime access and does not rely on env-var auth or the developer auth file.
...
- **Separation of dev vs runtime concerns**: SDK auth (human, short-lived) is stored in `~/.bv/auth.json`. Runner-mode auth comes from a local sqlite context store (`~/.bv/runner_context.db`) and never uses env-var auth. The runtime helpers in `bv.runtime` refuse to run unless `BV_SDK_RUN=1` is set by `bv run` or a runner context is present.

## 2.1 RPA vs Agentic Positioning (Documentation-Only)
- **RPA**: Deterministic, task-oriented automations with predictable inputs/outputs (queue-driven, system integration, repeatable workflows).
- **Agentic**: Goal-driven, multi-step orchestration that may involve adaptive decisioning, iterative calls, and higher trace fidelity requirements.
- **Current state**: The platform records the project type (`rpa` or `agent`) and propagates it through packages/processes/jobs, but execution behavior is currently shared across both types.
...
### `bv auth status`
- **What it does**: Reads auth from the runner sqlite context store (runner mode) or `~/.bv/auth.json` (developer mode), reports whether the token is expired and prints stored fields.
...
- **Runner mode**: Long-lived robot tokens are stored in the runner sqlite context store (`~/.bv/runner_context.db`). Runtime code reads that store per-process and never relies on env-var auth.
- **Runtime access boundaries**: `bv.runtime.assets` and `bv.runtime.queues` call `require_bv_run` and will raise unless `BV_SDK_RUN=1` (dev) or a runner sqlite context is present.
...
- **Version bumps**: `bv publish local` and `bv publish orchestrator` bump `bvproject.yaml` version. With `--dry-run`, the version is not written to disk.
